...
class MyApp(QtWidgets.QMainWindow, Ui_Window):
    ...
    def checkURLs(self):    	
    	...
    	df = pd.DataFrame(columns=('Course','Location','URL_Name','URL','Status_code','Remark','flag'))
                ...
                for urls in links:
                    for url in urls[1]:
                        if url[1].find("youtube.com") == -1 and url[1].find("youtu.be/") == -1:
    	                    ...
    	                    df = pd.DataFrame(np.array([[course_name, urls[0], url[0],url[1], status_code, comment, flag]]),
                                              columns=['Course','Location','URL_Name','URL','Status_code','Remark','flag']).append(df, ignore_index=True)
    	                else:
                            flag = 1
                            ...
                            df = pd.DataFrame(np.array([[course_name,urls[0], url[0], url[1], r.status_code, status, flag]]),
                                              columns=['Course','Location','URL_Name','URL','Status_code','Remark','flag']).append(df, ignore_index=True)

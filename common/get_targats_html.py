...
def get_all_links_and_name(page):
    urls =[]
    while  True:
        url,name, endpos = get_next_link_and_name(page)
        if url:
...
def get_next_link_and_name(page):
    a_link = page.find('<a ')
    start_link = page.find(' href="http', a_link+1) #http will exclude # button
    if start_link ==  -1:
        return None,None, 0
    ...
    end_a_link1 = page.find('>', end_quote)
    end_a_link2 = page.find('</a>',end_a_link1)
    name = page[end_a_link1 + 1 : end_a_link2]
    name = getContentParseHtml(name)
    
    return url,name, end_a_link2

def getContentParseHtml(html_txt):
